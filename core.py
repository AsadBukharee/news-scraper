...
def remove_duplicates():
    ...
    for key in news_keys:
        news_doc = redis_client.get(f'{key}').decode('utf-8')
        if news_doc:
            # if not isinstance(news_doc, dict):
            # with open("t.json", 'w') as f:
            #     f.writelines(f"{news_doc}")
            # with open("t.json", 'r') as f:
            #     user_data = json.load(f)
            # my_dict = ujson.loads(news_doc)
            # print(my_dict)
...
def store_local(website, name, data):
    try:
        directory = f'news_jsons/{website}/'
...
def store_redis(website, timestamp, value):
    try:
        ...
        if news_keys:
            ...
            news_keys = [timestamp]
        if store_local(website, f"{timestamp}.json", value):
...
def scrap_elaosboa():
    try:
        ...
        store_redis(website, timestamp=timestamp, value=value)
